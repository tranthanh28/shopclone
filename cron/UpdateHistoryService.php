...
if (time() > $CMSNT->site('check_time_cron_UpdateHistory5gsmm')) {
    if (time() - $CMSNT->site('check_time_cron_UpdateHistory5gsmm') < 5) {
...
$order_id = [];
foreach($CMSNT->get_list(" SELECT id_api FROM `order_service` WHERE `status` = 'Pending' OR `status` = 'In progress' OR `status` = 'Processing' OR `status` = '' ORDER BY RAND() ") as $get_order_id){
